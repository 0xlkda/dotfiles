...
vim.api.nvim_create_autocmd("FileChangedShell", {
  ...
  callback = function()
    autoread_saved_view = vim.fn.winsaveview()
  end,
})
...
function SyncScroll()
  local reversed = vim.fn.system("tmux show -gv @scroll-reversed 2>/dev/null"):gsub("%s+", "")
    == "1"
...
map(
  "n",
  "<leader>T",
  ":silent !~/code/dotfiles/tmux/theme.sh toggle<CR>",
  { noremap = true, desc = "Toggle light/dark theme" }
)
